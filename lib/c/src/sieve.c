...
sieve_t sieve_new(size_t size)
{
    ...
    for(size_t pos = 0; pos < size; pos++) sieve.state[pos] = true;
    sieve.state[0] = false;
    sieve.state[1] = false;
...
bool sieve_check(sieve_t *sieve, uint32_t num)
{
    return sieve->state[num];
}
