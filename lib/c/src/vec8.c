...
vec8_t vec8_new(size_t len, uint8_t fill) {
  vec8_t v = {.data = NULL, .cap = 256, .len = len};

  while (v.cap < len)
    v.cap *= 2;
  v.data = calloc(v.cap, sizeof(uint8_t));
  if (fill)
    memset(v.data, fill, len);
...
uint8_t vec8_get(vec8_t *v, size_t pos) { return v->data[pos]; }

void vec8_set(vec8_t *v, size_t pos, uint8_t data) { v->data[pos] = data; }
...
size_t vec8_len(vec8_t *v) { return v->len; }
