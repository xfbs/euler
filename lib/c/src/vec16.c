...
vec16_t vec16_new(size_t len, uint16_t fill) {
  vec16_t v = {.data = NULL, .cap = 256, .len = len};
...
uint16_t vec16_get(vec16_t *v, size_t pos) { return v->data[pos]; }

void vec16_set(vec16_t *v, size_t pos, uint16_t data) { v->data[pos] = data; }
...
size_t vec16_len(vec16_t *v) { return v->len; }
